...
{% include caption_img.html url="/assets/img/taxi-cab.png" width="400" description="Wut?" %}
...
{% include caption_img.html url="/assets/img/spacex-landing.jpg" width="600" description="Not these guys" %}

We need agents that learn to build on top of existing skills quickly in new, unseen environments. 
These agents should have very modular architecture, so one can plug in different skills in complex hierarchies.
...
There has been lots of work around this, notably in hierarchical and multi-task RL [TODO:citations]. 
For a detailed description of related work in this area, please take a look at our <a href="https://arxiv.org/abs/1711.11289">Arxiv paper</a>.
...
Ok, so how do we go about obtaining such reusability of skills? 
Let's say we have pre-trained policies for skill 1, $$\pi_1$$, and skill 2, $$\pi_2$$, and we want a policy for a task the requires composition of both skills, $$\pi_c$$. 
...
Ok, so what if we learned the function $$f$$?

How would we do that? 
If $$f$$ is differentiable, we could pass the policy gradient through it and adjust its parameters. 
...
Those we will keep fixed, as we want to be able to reuse the skills for future compositions. 
Let's write our equation down again with the parameters.

$$\pi_c : f(\pi_{\theta_1}, \pi_{\theta_2} \lvert \theta_c)$$


The final probability distribtion over actions, policy recommendations, from each skill may not contain enough information to blend them together. 
...
We would instead like to embed information about the state and the skill policy into a single layer of the skill network, and provide the embedding to $$f$$ to *learn to composition* as the task requires. So, the
final form of our composition function is

$$\pi_c : f(e_{\theta_1}, e_{\theta_2} \lvert \theta_c)$$

where $$e_{\theta_1}$$ is a function that generates embeddings for skill 1 and similarly for $$e_{\theta_2}$$. Fantastic! Now we can learn the function $$f$$ for any kind of composition, such as collect red object *while* evading green enemy, or evade green enemy *and* blue enemy, etc. But where do the embeddings come from?

### Phase 1
We break down the learning of our skill policies as follows,

$$\pi_1(a \lvert s) = \underbrace{\pi(a \lvert e_1(s); \theta_p)}_\text{policy layer}  \underbrace{p(e_1(s) \lvert s; \theta_1)}_\text{skill-state embedding}$$

In other words, there are two networks, one that learns a state embedding for each skill, and the other outputs a policy given any embedding. 
The pararmeters for the latter, $$\theta_p$$, are shared across all skills.
In practice, this can be achieved using the following architecture.

TODO: Instert gif on training the composenet architecture.
...
$$\pi_c(a \lvert s) = \underbrace{\pi(a \lvert e_c(s); \theta_p)}_\text{policy layer (fixed)}   \underbrace{p(e_c(s) \lvert e_1(s), e_2(s); \theta_c)}_\text{composed embedding}  \underbrace{p(e_1(s) \lvert s; \theta_1)}_\text{skill 1 (fixed)} \underbrace{p(e_2(s) \lvert s; \theta_2)}_\text{skill 2 (fixed)}$$
...
TODO: Insert gif of composition layer training.
...
$$\pi_c(a \lvert s) = \underbrace{\pi(a \lvert e_c(s); \theta_p)}_\text{policy layer (fixed)}   \underbrace{p(e_c(s) \lvert e_r(s), e_{and}(s); \theta_{while})}_\text{while composition}  \underbrace{p(e_r(s) \lvert s; \theta_r)}_\text{skill 1 (fixed)} \underbrace{p(e_{and}(s) \lvert e_{\neg g}, e_{\neg b}; \theta_{and})}_\text{and composition} \underbrace{p(e_{\neg g}(s) \lvert s; \theta_{\neg g})}_\text{skill 2 (fixed)} \underbrace{p(e_{\neg b}(s) \lvert s; \theta_{\neg b})}_\text{skill 3 (fixed)}$$


and the network,

TODO: insert hierarchical network here.
